...
import { Lock, X, AlertCircle, KeyRound, ArrowLeft } from 'lucide-react';
...
export default function PinVerificationModal({
  ...
  const handleRecoverSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const isSuccess = await invoke<boolean>('verify_recovery_key', { key: recoveryKey });
      if (isSuccess) {
        // Recovery reset successful!
        alert('Yönetici PIN koruması başarıyla sıfırlandı. İşlem kaldığı yerden devam ediyor.');
        onSuccess();
        onClose();
      } else {
        setError('Hatalı kurtarma anahtarı. Lütfen tekrar deneyin.');
      }
    } catch (err: any) {
      setError(err?.toString() || 'Sıfırlama hatası oluştu.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-zinc-950 border border-zinc-850 rounded-2xl shadow-2xl overflow-hidden p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-amber-500/10 border border-amber-500/20 rounded-xl text-amber-500">
              {isRecovering ? <KeyRound className="w-5 h-5" /> : <Lock className="w-5 h-5" />}
            </div>
            <div>
              <h3 className="text-md font-bold text-zinc-150">
                {isRecovering ? 'Yönetici Kurtarma' : 'Yönetici Doğrulaması'}
              </h3>
              <p className="text-xs text-zinc-400">
                {isRecovering ? 'Kurtarma anahtarı ile şifre sıfırlama' : 'Yetkisiz erişimi engellemek için PIN girin'}
              </p>
            </div>
          </div>
          <button
        ...
        {isRecovering ? (
          ...
          <form onSubmit={handleRecoverSubmit} className="space-y-4">
            <div className="p-3 bg-amber-950/20 border border-amber-900/40 text-amber-300 text-[11px] rounded-xl leading-relaxed">
              Lütfen ilk kurulumda veya şifre oluşturulurken size verilen 16 haneli kurtarma anahtarını girin. Doğru anahtar girildiğinde yönetici PIN koruması kaldırılacaktır.
            </div>

            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-zinc-400">Kurtarma Anahtarı</label>
              <input
                type="text"
                autoFocus
                required
                ...
                className="w-full px-4 py-3 bg-zinc-900/60 border border-zinc-800/80 rounded-xl text-center text-md font-mono uppercase text-zinc-100 placeholder-zinc-650 focus:border-amber-600 outline-none transition-all"
              />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              <button
                ...
                disabled={loading}
                className="px-4 py-2.5 bg-zinc-900 hover:bg-zinc-850 active:bg-zinc-900 text-zinc-350 hover:text-zinc-200 border border-zinc-800 hover:border-zinc-750 transition-colors text-xs font-bold rounded-xl cursor-pointer disabled:opacity-50 flex items-center justify-center gap-1.5"
              >
              ...
              <button
                ...
                className="flex-1 px-4 py-2.5 bg-amber-600 hover:bg-amber-500 active:bg-amber-700 text-white font-bold text-xs rounded-xl transition-all cursor-pointer shadow-lg shadow-amber-600/10 flex items-center justify-center gap-1.5"
              >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="text-center pt-2">
              <button
                ...
                className="text-[11px] font-semibold text-amber-500 hover:text-amber-400 hover:underline cursor-pointer transition-colors"
              >
                PIN Kodumu Unuttum
              </button>
            </div>
