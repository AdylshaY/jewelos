...
export default function VaultDashboard() {
  ...
  const handleCloseVault = async () => {
    if (window.confirm(`${selectedDate} tarihli kasayı kapatmak istediğinize emin misiniz? Bu işlem geri alınamaz.`)) {
      try {
  ...
  const getAssetColor = (assetType: string) => {
    switch (assetType) {
      case 'TRY': return 'border-l-zinc-650';
      case 'USD': return 'border-l-blue-650';
      case 'EUR': return 'border-l-indigo-650';
      case 'FINE_GOLD': return 'border-l-amber-500';
      case 'PRODUCT': return 'border-l-yellow-600';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-5 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold text-zinc-100">Günlük Kasa Defteri</h2>
            <p className="text-xs text-zinc-400">Günlük kasa durumunu, kurlarını ve nakit akışını izleyin</p>
          </div>
        </div>
      ...
      {!vaultStatus ? (
        ...
        <div className="flex flex-col items-center justify-center p-12 text-center bg-zinc-900/30 border border-zinc-800/70 border-dashed rounded-3xl min-h-[300px]">
          ...
          <h3 className="text-lg font-semibold text-zinc-200 mb-1">{selectedDate} Tarihli Kasa Açılmamış</h3>
          <p className="text-sm text-zinc-450 max-w-sm mb-6">Bu tarihte henüz gün sonu/kasa defteri başlatılmamış. İşlem yapabilmek için kasayı açmalısınız.</p>
          <button
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1 p-6 bg-gradient-to-br from-amber-500/10 via-amber-600/5 to-transparent border border-amber-500/20 rounded-2xl relative overflow-hidden flex flex-col justify-between min-h-[140px]">
              <div>
                <span className="text-xs font-semibold text-amber-500/80 uppercase tracking-wider">Kasa Toplam Has Altın Değeri</span>
                <h3 className="text-3xl font-black text-amber-500 mt-2">
                  {summary ? formatCurrency(summary.total_fine_gold, 'FINE_GOLD') : '0.00 g'}
                </h3>
              </div>
              <p className="text-[11px] text-zinc-400 mt-4 border-t border-amber-500/15 pt-2">
                Nakit döviz ve altın varlıklarının güncel alış kurları üzerinden toplam altın karşılığı.
              </p>
            </div>
            ...
            {summary?.rates && (
              <div className="md:col-span-2 p-5 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl backdrop-blur-md flex flex-col justify-between">
                <div>
                  <h4 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-3.5">Aktif Kurlar</h4>
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl">
                      <span className="text-[10px] text-zinc-450 block font-medium">USD / TRY</span>
                      <span className="text-sm font-semibold text-zinc-200 mt-1 block">Alış: {summary.rates.usd_buy}</span>
                      <span className="text-xs text-zinc-400 block">Satış: {summary.rates.usd_sell}</span>
                    </div>
                    <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl">
                      <span className="text-[10px] text-zinc-450 block font-medium">EUR / TRY</span>
                      <span className="text-sm font-semibold text-zinc-200 mt-1 block">Alış: {summary.rates.eur_buy}</span>
                      <span className="text-xs text-zinc-400 block">Satış: {summary.rates.eur_sell}</span>
                    </div>
                    <div className="bg-zinc-950/40 border border-zinc-850 p-3 rounded-xl">
                      <span className="text-[10px] text-zinc-450 block font-medium">ALTIN (24K)</span>
                      <span className="text-sm font-semibold text-amber-500 mt-1 block">Alış: {summary.rates.gold_buy}</span>
                      <span className="text-xs text-zinc-400 block">Satış: {summary.rates.gold_sell}</span>
                    </div>
                  </div>
                </div>
                {vaultStatus.status === 'open' && (
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Varlık Bakiyeleri</h3>
              ...
              {summary?.balances.map((b) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-[11px] font-semibold text-zinc-300">{getAssetLabel(b.asset_type)}</span>
                    {getAssetIcon(b.asset_type)}
                  </div>
                  <div>
                    <span className="text-[10px] text-zinc-450 block">Açılış: {formatCurrency(b.opening_balance, b.asset_type)}</span>
                    <span className="text-lg font-bold text-zinc-100 block mt-1">{formatCurrency(b.closing_balance, b.asset_type)}</span>
                    {b.asset_type !== 'FINE_GOLD' && b.asset_type !== 'PRODUCT' && (
                      <span className="text-[10px] text-amber-500/80 block mt-0.5">≈ {b.fine_gold_value.toFixed(2)} g Has</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2 mt-3 pt-2 border-t border-zinc-850/50 text-[9px] text-zinc-450">
                    <span className="text-emerald-400 font-medium">Giriş: {formatCurrency(b.in_amount, b.asset_type)}</span>
                    <span className="text-rose-400 font-medium">Çıkış: {formatCurrency(b.out_amount, b.asset_type)}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="p-5 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl backdrop-blur-md space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-semibold text-zinc-200">Günün Kasa Hareketleri</h3>
              {vaultStatus.status === 'open' && (
                <button
                ...
                <tbody className="divide-y divide-zinc-850/50 text-zinc-300">
                  {summary?.transactions.length === 0 ? (
                    ...
                    summary?.transactions.map((tx) => (
                      <tr key={tx.id} className="hover:bg-zinc-950/20 transition-colors">
                        <td className="py-3 font-semibold text-zinc-200">{getAssetLabel(tx.asset_type)}</td>
                        <td className="py-3">
                        ...
                        <td className="py-3 font-medium">{formatCurrency(tx.amount, tx.asset_type)}</td>
                        <td className="py-3 font-semibold text-amber-500">{tx.fine_gold_gram.toFixed(2)} g</td>
