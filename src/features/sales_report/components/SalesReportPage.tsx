...
export default function SalesReportPage({ theme }: SalesReportPageProps) {
  const { report, loading, fetchReport, translateCategory, getPurityLabel } = useSalesReport();

  const [subTab, setSubTab] = useState<'sales' | 'vault'>('sales');
  const [dateFrom, setDateFrom] = useState('');
  const [dateTo, setDateTo] = useState('');
  const [searchTerm, setSearchTerm] = useState('');

  // Fetch report on mount and when dates change (only if sales tab is active)
  useEffect(() => {
    if (subTab === 'sales') {
      fetchReport(dateFrom || undefined, dateTo || undefined);
    }
  }, [fetchReport, dateFrom, dateTo, subTab]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-5 bg-zinc-900/40 border border-zinc-800/80 rounded-2xl backdrop-blur-md">
        <div>
          <h2 className="text-xl font-bold text-zinc-100 flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-amber-500" /> Raporlar & Analiz
          </h2>
          <p className="text-xs text-zinc-400">Tarih bazlı satış analizi, dükkan giderleri ve aylık istatistikler</p>
        </div>
      ...
      {/* Sub-tabs Navigation */}
      <div className="flex gap-2 p-1 bg-zinc-900/40 border border-zinc-800/80 rounded-xl w-fit backdrop-blur-md">
        <button
          onClick={() => setSubTab('sales')}
          className={`px-4 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer ${
            subTab === 'sales'
              ? 'bg-zinc-800/60 text-zinc-100 border border-zinc-700/55 shadow-inner shadow-black/10'
              : 'text-zinc-450 hover:text-zinc-300 hover:bg-zinc-900/40 border border-transparent'
          }`}
        >
          Satış Analizi
        </button>
      ...
      {subTab === 'sales' ? (
        ...
        <VaultReportTab dateFrom={dateFrom} dateTo={dateTo} theme={theme} />
      )}
    </div>
  );
}
